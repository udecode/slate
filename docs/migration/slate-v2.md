...
- `slate` owns the editor runtime, document model, transactions, state fields,
  and pure helper APIs.
- `slate-dom` owns DOM resolution, clipboard helpers, hotkeys, and browser
  environment helpers.
- `slate-react` owns `<Slate>`, `<Editable>`, render primitives, React hooks,
...
For pure data checks, import the helper API from `slate`.

```tsx
import { NodeApi, RangeApi, TextApi } from 'slate'
...
```tsx
import { useEditorState } from 'slate-react'
...
```tsx
import { useElementSelected } from 'slate-react'
...
```tsx
import { useElementPath } from 'slate-react'
...
```tsx
import { useSlateHistory } from 'slate-react'
...
```tsx
import { type SlateChange } from 'slate-react'
...
```tsx
import { useSlateRootChrome, useSlateRootEditor } from 'slate-react'
...
import { DOMCoverage, dom, Hotkeys, isHotkey } from 'slate-dom'
import {
  Editable,
  Slate,
  createReactEditor,
...
- [Slate React Hooks](../libraries/slate-react/hooks.md)
