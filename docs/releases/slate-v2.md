...
`slate-react` exposes decoration-source and annotation-store hooks such as
...
| Package             | Purpose                                                                                                                 |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `slate`             | editor runtime, document model, operations, transactions, state fields, schema, pure helper APIs                        |
| `slate-dom`         | DOM bridge, clipboard bridge, selection conversion, hotkeys, DOM coverage helpers                                       |
...
`slate-history` is a runtime extension instead of a wrapper function.
...
`slate-hyperscript` remains the fixture-friendly package for tests:
...
`slate-layout` adds experimental layout readers and React page surfaces:
...
Use `slate` for the data model, editor runtime, operations, transactions, helper
...
Use `slate-react` for the React editor, provider, editable surface, render
...
Use `slate-dom` directly when framework code needs DOM bridge helpers without
React.
...
Use `slate-history` when building an editor manually or when test code needs
...
Use `slate-hyperscript` for JSX fixtures.
