...
Element nodes behave differently depending on the [Slate editor's schema](./editor.md#schema-behavior). An element can be:
...
```tsx
import type { RenderVoidProps } from 'slate-react'
...
```tsx
import { useElementSelected, type RenderVoidProps } from 'slate-react'
