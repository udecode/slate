...
| Browser surface changed | yes | Run focused Playwright proof | `PLAYWRIGHT_RETRIES=0 PLAYWRIGHT_WORKERS=1 bunx playwright test playwright/integration/examples/pagination.test.ts --project=chromium --grep "keeps fast staged text after insert breaks|keeps staged burst typing responsive|keeps fast burst typing intact|keeps middle-document typing responsive"` passed 4/4 |
