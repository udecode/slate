...
const getQueryParam = (page: Page, key: string) =>
  page.evaluate((paramKey) => {
    return new URL(window.location.href).searchParams.get(paramKey)
  }, key)
...
test.describe('example query controls', () => {
  test('stores search highlighting control state in the URL', async ({
    ...
    await openExample(page, 'search-highlighting', {
      query: { q: 'decorations' },
      ready: {
        editor: 'visible',
  ...
  test('stores android test selector state in the URL', async ({ page }) => {
    await openExample(page, 'android-tests', {
      query: { test: 'insert' },
      ready: {
        editor: 'visible',
  ...
  test('loads pagination controls from the URL and stores updates', async ({
    ...
    await openExample(page, 'pagination', {
      query: {
        debug: true,
        margins: 72,
        media_height: 320,
