...
import { type Descendant, NodeApi, type Operation, type Range } from 'slate'
...
const readInitialRoomName = () => {
  ...
  return new URLSearchParams(window.location.search).get('room') ?? DEFAULT_ROOM
}
...
const selectedText = (editor: CustomEditor) =>
  editor.api.dom.getWindow().getSelection()?.toString().replaceAll('\uFEFF', '')
...
const firstBlockIsQuote = (editor: CustomEditor) => {
  ...
  return firstBlock && 'type' in firstBlock && firstBlock.type === 'block-quote'
}
