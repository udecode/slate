...
const isCacheableStatus = (status) => status === 'failed' || status === 'passed'
...
export const renderFailuresMarkdown = ({
  ...
  const lines = [`# Integration run ${runId}`, '', `Exit code: ${exitCode}`, '']
...
export const renderPickupMarkdown = ({ failuresText = '', status }) => {
  ...
  if (status.failuresPath) lines.push(`- Failure inbox: ${status.failuresPath}`)
...
const printStatus = async (runIdArg) => {
  ...
  const live = status.status === 'running' && isProcessRunning(status.workerPid)
  const liveSuffix =
