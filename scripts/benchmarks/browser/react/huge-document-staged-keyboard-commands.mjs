...
const requireFullDOMSurface =
  process.env.SLATE_STAGED_COMMANDS_REQUIRE_FULL_DOM !== '0'
...
const waitForStagedSurface = async (page) => {
  await page.waitForFunction(
    ({ expectedBlocks, requireFullDOMSurface }) => {
      const root = document.querySelector('[data-slate-editor="true"]')
      ...
      if (effectiveStrategy !== 'staged') {
        return false
      }

      if (!requireFullDOMSurface) {
    ...
    { expectedBlocks: blocks, requireFullDOMSurface },
    { timeout: 30_000 }
  )
}
...
const summarizeSurface = ({ selectAllDeleteSamples, verticalSamples }) => ({
...
const run = async () => {
  if (surfaces.length === 0) {
    throw new Error('SLATE_STAGED_COMMANDS_SURFACES selected no known surfaces')
  }
