import { NodeApi, PathApi, type Point, PointApi, type Range } from 'slate'
...
const clampPlainVerticalLargeDocumentTarget = ({
  ...
  const anchorBeforeFocus = PointApi.isBefore(selection.anchor, selection.focus)
...
const isDirectionalPlainVerticalTarget = ({
  ...
  reverse ? PointApi.isBefore(target, source) : PointApi.isAfter(target, source)
...
const resolveVisualLineTargetPoint = ({
  ...
  try {
    ...
    const currentLine =
      lineRects[currentLineIndex] ?? (reverse ? lineRects[0] : lineRects.at(-1))
