...
export const getSlatePageLayoutFragments = (
  ...
  for (const ancestorPathKey of getAncestorPathKeys(path)) {
    addFragmentUnitMatches(matchedUnits, index.unitsByPath.get(ancestorPathKey))
  }
...
const createEstimatedLines = (
  ...
  if (block.lines && block.lines.length > 0) {
    return block.lines.map((line) => {
      const runs =
        line.runs ??
        (block.runs ? createEstimatedLineRuns(block, line.start, line.end) : [])
...
export const pretextPageLayoutEngine = ({
  ...
  const createRichInlineLines = (
    ...
    const sourceItems = block.runs.map((run) => {
      ...
      const sourceEnd = run.text.length - getTrailingCollapsibleLength(run.text)
      const text =
...
export const paginateSlatePageLayoutBlocks = ({
  ...
  measuredBlocks.forEach((block) => {
    ...
    while (remainingLines > 0) {
      ...
      const height =
        lineCount * block.lineHeight + (isLastFragment ? block.spacingAfter : 0)
