...
Replace the legacy React plugin and renderer surface with the Slate v2 React runtime.
...
- Replace `withReact`, legacy `ReactEditor` value imports, `DefaultElement`, `DefaultLeaf`, `DefaultText`, `useSlate`, `useSlateStatic`, `useSlateSelector`, `useSlateSelection`, `useSelected`, `useFocused`, `useReadOnly`, and `useComposing` with the v2 components and hooks exported from `slate-react`.
